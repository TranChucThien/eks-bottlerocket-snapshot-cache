...
Tạo EBS snapshot mới mỗi khi có release mới (mỗi lần build image mới → tạo snapshot tương ứng).
...
| **Chi phí** | Cao — mỗi release tạo 1 snapshot mới, tốn chi phí lưu trữ EBS snapshot và thời gian chạy workflow |
...
| | |
|---|---|
| **Cách hoạt động** | Tạo snapshot theo lịch cố định, các release giữa các window sẽ tận dụng shared layers từ snapshot cũ |
